...
Поэтому мы:
  ...
  4. Для каждого дня получили его представление с учетом предыдущей истории с помощью скользящего среднего: $h_t = \alpha * h_{t-1} + (1 - \alpha) * v_t$ ($v_t$ - представление текущего дня на шаге 3).\
